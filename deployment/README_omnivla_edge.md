...
1. **제어 명령이 항상 작게(안전 캡에 붙어서) 나오는 문제** — `waypoint_to_control()`이 목표 waypoint의
   ...
   일부러 보수적으로 잡아둔 값입니다.
2. **대시보드 지도가 실제 heading과 안 맞게 회전하는 문제** — `poll_frodobot()`이 FrodoBot의
   `orientation`(나침반 방위각으로 추정)을 우리 좌표계로 바꿀 때 90도 보정이 빠져 있던 걸 수정함.
   그래도 안 맞으면 `--heading_sign -1`로 실행해서 방향을 뒤집어보고, 대시보드에 표시되는
   `raw orientation` 값과 실제 로봇이 보는 방향을 비교하며 `omnivla_edge_deploy.py`의
   `HEADING_OFFSET_DEG` 상수를 조정할 것. **로봇을 정지시켜두고 여러 방향으로 돌려보면서 대시보드
   지도가 heading-up으로 정렬되는지 확인하는 캘리브레이션을 배포 전 반드시 해볼 것.**
